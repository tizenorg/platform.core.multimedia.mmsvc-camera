...
void _mmsvc_camera_disp_capturing_cb(camera_image_data_s* image, camera_image_data_s* postview, camera_image_data_s* thumbnail, void *user_data)
{
	...
	LOGD("bohandle_ptr : 0x%x, export_bo : %d, tSize : %d", transport_info.bo_handle.ptr,
													transport_info.bo,
...
int mmsvc_camera_disp_create(Client client)
{
	...
	handle = (int)camera;
	LOGD("device_type : %d, handle : 0x%x", device_type, handle);
...
int mmsvc_camera_disp_set_display(Client client)
{
	...
	LOGD("handle : 0x%x, display_type : %d, display_handle : 0x%x", handle, display_type, display_handle);
