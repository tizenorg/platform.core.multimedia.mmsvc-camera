...
void _camera_dispatcher_preview_cb(MMCamcorderVideoStreamDataType *stream, void *user_data)
{
	...
	if (stream->bo[0] == NULL) {
		...
		data_bo = tbm_bo_alloc(muse_camera->bufmgr, data_size, TBM_BO_DEFAULT);
		if (data_bo == NULL) {
			LOGE("data_bo alloc failed");
			goto _PREVIEW_CB_ERROR;
		}

		data_bo_handle = tbm_bo_map(data_bo, TBM_DEVICE_CPU, TBM_OPTION_READ | TBM_OPTION_WRITE);
		if (data_bo_handle.ptr == NULL) {
			LOGE("data_bo map Error!");
			goto _PREVIEW_CB_ERROR;
		}
...
int camera_dispatcher_get_display_reuse_hint(muse_module_h module)
{
	...
	ret = legacy_camera_get_display_reuse_hint(muse_camera->camera_handle, (bool *)&get_hint);
	if (ret == CAMERA_ERROR_NONE) {
