...
int camera_dispatcher_attr_set_pan(muse_module_h module)
{
	...
	muse_camera_msg_get(set_pan_degree, muse_core_client_get_msg(module));

	LOGD("handle : 0x%x", muse_camera);

	ret = legacy_camera_attr_set_pan(muse_camera->camera_handle, set_move_type, set_pan_degree);
...
int camera_dispatcher_attr_get_pan(muse_module_h module)
{
	...
	ret = legacy_camera_attr_get_pan(muse_camera->camera_handle, &get_pan_degree);
	if (ret == CAMERA_ERROR_NONE) {
		muse_camera_msg_return1(api, class, ret, module, INT, get_pan_degree);
	} else {
		muse_camera_msg_return(api, class, ret, module);
	}
...
int camera_dispatcher_attr_set_tilt(muse_module_h module)
{
	...
	muse_camera_msg_get(set_move_type, muse_core_client_get_msg(module));
	muse_camera_msg_get(set_tilt_degree, muse_core_client_get_msg(module));

	LOGD("handle : 0x%x", muse_camera);

	ret = legacy_camera_attr_set_tilt(muse_camera->camera_handle, set_move_type, set_tilt_degree);
...
int camera_dispatcher_attr_get_tilt(muse_module_h module)
{
	...
	ret = legacy_camera_attr_get_tilt(muse_camera->camera_handle, &get_tilt_degree);
	if (ret == CAMERA_ERROR_NONE) {
		muse_camera_msg_return1(api, class, ret, module, INT, get_tilt_degree);
	} else {
		muse_camera_msg_return(api, class, ret, module);
	}
