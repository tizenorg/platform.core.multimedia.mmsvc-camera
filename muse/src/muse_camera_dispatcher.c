...
int camera_dispatcher_attr_set_flash_mode(muse_module_h module)
{
	...
	if (ret == CAMERA_ERROR_NONE) {
		if (set_mode > CAMERA_ATTR_FLASH_MODE_OFF)
			muse_core_client_set_value(module, "flash_state", 1);
		else
			muse_core_client_set_value(module, "flash_state", 0);
	}
...
int camera_dispatcher_attr_get_flash_state(muse_module_h module)
{
	int ret = CAMERA_ERROR_NONE;
	muse_camera_handle_s *muse_camera = NULL;
	camera_attr_flash_state_e get_flash_state = CAMERA_ATTR_FLASH_STATE_NOT_USED;
	muse_camera_api_e api = MUSE_CAMERA_API_ATTR_GET_FLASH_STATE;
	muse_camera_api_class_e class = MUSE_CAMERA_API_CLASS_IMMEDIATE;

	LOGD("flash_state\n");

	muse_core_client_get_value(module, "flash_state", &get_flash_state);
