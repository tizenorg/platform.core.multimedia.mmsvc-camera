...
static int _camera_remove_export_data(muse_module_h module, int key, int remove_all)
{
	...
	while (tmp_list) {
		...
		if (export_data) {
			if ((export_data->key == key && NEED_RELEASE_BUFFER(muse_camera->preview_cb_flag)) || remove_all) {
...
void _camera_dispatcher_preview_cb(MMCamcorderVideoStreamDataType *stream, void *user_data)
{
	...
	*/
	...
	if (CHECK_PREVIEW_CB(muse_camera, PREVIEW_CB_TYPE_EVAS)) //if (!(muse_camera->preview_cb_flag & PREVIEW_CB_TYPE_EVAS)) {
...
int camera_dispatcher_set_display(muse_module_h module)
{
	...
	if (type == CAMERA_DISPLAY_TYPE_OVERLAY) {
		...
		if (type == CAMERA_DISPLAY_TYPE_EVAS) {
			...
			muse_camera->preview_cb_flag |= PREVIEW_CB_TYPE_EVAS;
			LOGD("muse_camera->preview_cb_flag = 0x%08X", muse_camera->preview_cb_flag);
		}
...
int camera_dispatcher_set_preview_cb(muse_module_h module)
{
	...
	muse_camera->preview_cb_flag |= PREVIEW_CB_TYPE_USER;
	LOGD("ret : 0x%x", ret);
...
int camera_dispatcher_unset_preview_cb(muse_module_h module)
{
	...
	muse_camera->preview_cb_flag &= ~PREVIEW_CB_TYPE_USER;
	if (muse_camera->preview_cb_flag & PREVIEW_CB_TYPE_EVAS)
