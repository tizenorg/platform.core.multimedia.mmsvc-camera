...
int camera_dispatcher_get_flash_state(muse_module_h module)
{
	...
	if (device_type == CAMERA_DEVICE_CAMERA0)
		muse_core_client_get_value(module, "primary_flash_state", &get_flash_state);
	else
		muse_core_client_get_value(module, "secondary_flash_state", &get_flash_state);
...
int camera_dispatcher_attr_set_flash_mode(muse_module_h module)
{
	...
	if (ret == CAMERA_ERROR_NONE) {
		...
		if (ret == CAMERA_ERROR_NONE) {
			if (device_type == CAMERA_DEVICE_CAMERA0)
				muse_core_client_set_value(module, "primary_flash_state", set_mode>0?1:0);
			else
				muse_core_client_set_value(module, "secondary_flash_state", set_mode>0?1:0);
		}
	}
